<template>
  <Modal v-model="visible" class="model-detail" :closable="false" :footer-hide="true">
...
const getDotaItemDetail = () => {
  ...
  return new Promise(resolve => {
    jsonp({
      ...
      callback(data) {
        for (const [prop, value] of Object.entries(data.itemdata)) {
          itemList.push(value);
        }
        resolve(itemList);
      }
...
const showModal = itemId => {
  getDotaItemDetail().then(itemList => {
    const item = itemList.find(item => item.id.toString() === itemId.toString());
